...
DevOps / Systems Engineer & scott.crooks@gmail.com \\
http://github.com/sc250024 & https://linkedin.com/in/jshcrooks \\
\end{tabular*}
\\
...
\item
    ...
    \ressubheading{Humboldt-Institut}{Berlin, Germany}{Intensive German Language Course}{June 2017 to October 2017}

    \begin{itemize}

        \resitem{Intensive German language course to obtain B1 level of German in order to fulfill the German language requirement for the EU Blue Card. Humboldt-Institut schedules 30 hours per week of instruction in all aspects of German language skills: reading, writing, speaking, and listening.}

    \end{itemize}

    \ressubheading{National Taiwan University}{Taipei, Taiwan}{International Chinese Language Program (ICLP)}{September 2014 to June 2015}
...
\item
    ...
    \ressubheading{Oregon State University}{Corvallis, Oregon, USA}{B.Sc. Chemical Engineering}{September 2004 to June 2009}
...
\item {\bf Programming Languages:} BASH / Shell, PHP, Python, SQL, Jinja, \LaTeX

\item {\bf Spoken Languages:} English (native), Mandarin Chinese (HSK 4), German (B1)

\item {\bf Technical Software}
    \begin{itemize}
        \resitem{BI \& Monitoring:} Check\_MK, DataDog, Grafana, Icinga, Looker, Nagios, New Relic, Piwik, TICK Stack
        \resitem{Build \& CI:} AWS CloudFormation, CircleCI, CodeClimate, DeployBot, Docker Swarm, Jenkins
        \resitem{Collaboration:} Asana, Confluence, JIRA, MatterMost, Slack, Trello
        \resitem{Config \& Provisioning:} Packer, Puppet, SaltStack, Vagrant
        \resitem{Databases:} MariaDB / MySQL, MongoDB, PostgreSQL
        \resitem{Infrastructure:} GNU/Linux (all distros), Active Directory, GlusterFS, HAProxy, LDAP, NGINX, OpenVPN, PowerDNS, Unbound \& NSD DNS
        \resitem{SCM \& Repo Management:} Docker Registry, Git, Gitlab, PHP Packagist, Sonatype Nexus (Maven), SVN
        \resitem{Virtualization \& Cloud IaaS:} Amazon AWS, Citrix XenServer, OpenStack, VMware ESXi
    \end{itemize}
...
\item
    ...
    \ressubheading{Vacasa, LLC}{Portland, Oregon, USA}{DevOps Engineer}{December 2015 to Present}
    \linebreak
    \linebreak
    \emph{Rental property management company providing comprehensive management services for over 5,000 vacation rentals. Member of a DevOps team that used SCRUM and Agile methods for collaboration between team members and developers.}
    \begin{itemize}
            \resitem{Initiated an OpenVPN project for engineering and analytics department. Around 40 members of the company use the VPN solution that is powered by Docker, and conforms to the NSA Commercial National Security Algorithm Suite cryptographic standards. Original implementation is open-sourced on GitHub here: https://github.com/helin24/openvpn-php-access}

            \resitem{Performed a complete database conversion to UTF8 encoding on the company's primary MariaDB database using Amazon Database Migration Service (DMS). Wrote scripts in PHP to check for data integrity between the Latin1 / UTF8 versions of the data while running the migration.}

            \resitem{Setup SaltStack as a configuration management tool for Vacasa's non-Docker based infrastructure; solution utilized SaltCloud as a server provisioning method. Contributions were also made to the OpenVPN and PHP SaltStack formulas on Github.}

            \resitem{Migrated Vacasa's 5 services and all associated servers from EC2 Classic to a VPC-based network with minimal downtime, and with the potential for fallback if the migration proceeded incorrectly.}

            \resitem{Responsible as the primary maintainer for 5 of Vacasa's Docker-based services: Mattermost, OpenVPN, Sentry, the IP and Network Planning tool, and MONyog. All of the services used Docker Swarm deployments through Jenkins (or Portainer for privileged containers), and CI testing with CircleCI.}
...
\item
    ...
    \ressubheading{Frito-Lay, Inc.}{Vancouver, Washington, USA}{Project Engineer / Capital Expenditures Intern}{June 2008 to January 2009}
    \linebreak
    \linebreak
...
\underline{Vacasa, LLC}
\linebreak

\begin{itemize}

    \item
        \ressubheading{DataDog Integration Project}{Portland, Oregon, USA}{DevOps Engineer}{December 2016 to June 2017}
        \linebreak
        \linebreak
        ...
        \linebreak
        \linebreak
        Integrated DataDog monitoring into Vacasa's existing plain EC2 and Docker Swarm servers.

        \underline{Responsibilities}
        \begin{itemize}
            \resitem{Setup monitoring checks in DataDog to monitor our MySQL RDS servers with the following metrics: queries per second, replica lag, cached query utilization, CPU and RAM utilization, and number of slow queries per hour.}
            \resitem{For our Docker-based OpenVPN containers, set up checks for number of connected clients, as well as amount of available entropy for the host server (important for random-number generation needed for cryptography.}
            \resitem{Deployed Datadog agent to all servers using SaltStack, and contributed to the official SaltStack formula for the deployment of the DataDog agent onto Debian/Redhat-based servers: https://github.com/DataDog/datadog-formula/pull/15}
        \end{itemize}

        \underline{Technology Utilized}
        \begin{itemize}
            \resitem{DataDog Linux Agent, SaltStack}
        \end{itemize}

    \item
        \ressubheading{OpenVPN Research \& Deployment}{Portland, Oregon, USA}{DevOps Engineer}{April 2016 to June 2017}
        \linebreak
        \linebreak
        ...
        \linebreak
        \linebreak
        Researched, setup, and implemented two iterations of a Docker-based VPN service using OpenVPN NL and later OpenVPN 2.4.0+. Initial solution utilized OpenVPN NL, which is a fork of OpenVPN used by the Dutch government, as well as the Authy two-factor authentication service. The second version contained complete integration with OneLogin, which Vacasa was using as a single sign-on (SSO) provider, as well as load balancing, and health checks using AWS Route 53.
